...
# Liste des rôles autorisés pour tous les comptes
ROLES_AUTORISES = ['demandeur', 'atelier', 'superviseur', 'admin', 'root']
# CONFIGURATION FLASK POUR TON DOSSIER
app = Flask(
    __name__,
    static_folder='../frontend',    # <- Le dossier avec tous tes JS/CSS/images
    static_url_path=''              # <- Accessible depuis /logo-atelier.png etc.
)
...
def init_db():
    ...
    ''')
    # MIGRATION pour ajout de colonnes sur anciennes BDD :
    try:
        c.execute("ALTER TABLE demandes ADD COLUMN salle TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        c.execute("ALTER TABLE demandes ADD COLUMN batiment TEXT")
    except sqlite3.OperationalError:
        pass
    try:
        c.execute("ALTER TABLE demandes ADD COLUMN date TEXT")
    except sqlite3.OperationalError:
        pass
...
def create_compte():
    # 1. Vérifie que l'admin est connecté (sécurité)
...
def login_admin():
    ...
    if row and check_password_hash(row[0], data['password']):
        if row[1] == "admin" or row[2] == 1:
            session.permanent = True  # <-- Très important !
...
def api_demandes():
    ...
    if request.method == 'POST':
        ...
        date = data.get('date', '')
        statut = 'Nouveau'  # Statut forcé à "Nouveau"
        ...
        user_id = session.get('user_id')
        # On récupère le filtre statut dans les paramètres d'URL (?statut=xxx)
        statut = request.args.get('statut')

        base_query = 'SELECT d.id, d.titre, d.description, d.salle, d.batiment, d.date, d.statut, d.date_crea, c.username FROM demandes d LEFT JOIN comptes c ON d.user_id = c.id'
        params = []
        conditions = []

        # atelier et SUPERVISEUR voient tout (peuvent filtrer par statut)
...
def maj_demande(demande_id):
    ...
    c.execute('''
        UPDATE demandes
        SET statut=?, titre=?, description=?
        WHERE id=?
    ''', (statut, titre, description, demande_id))
    conn.commit()
    conn.close()
    return jsonify({'ok': True})
...
# ----------- ROUTES HTML (rendu dynamique Jinja) ---------------
